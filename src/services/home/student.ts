...
class StudentService {
  private virtualDb: Student[] = Array.from({ length: 25 }).map((_, index) => {
  ...
  async getActive(): Promise<Student[]> {
    await this.delay();
    return this.virtualDb.filter(s => !s.isDeleted);
  }
  ...
  async getById(id: string): Promise<Student | undefined> {
    await this.delay();
    return this.virtualDb.find(s => s.id === id);
  }

  async getStats(): Promise<StudentStats> {
    await this.delay();
    const activeStudents = this.virtualDb.filter(s => !s.isDeleted);
  ...
  async create(studentData: Omit<Student, 'id' | 'isDeleted'>): Promise<Student> {
    await this.delay();
    ...
    this.virtualDb.unshift(newStudent);
    return newStudent;
  }

  async update(id: string, updateData: Partial<Student>): Promise<Student> {
    await this.delay();
    const index = this.virtualDb.findIndex(s => s.id === id);
    if (index === -1) throw new Error('Student not found');
    
    this.virtualDb[index] = { ...this.virtualDb[index], ...updateData };
    return this.virtualDb[index];
  }
  ...
  async softDelete(id: string): Promise<void> {
    ...
    const index = this.virtualDb.findIndex(s => s.id === id);
    if (index !== -1) {
      this.virtualDb[index].isDeleted = true;
      this.virtualDb[index].deletedAt = new Date().toLocaleString();
    }
  }
  ...
  async restore(id: string): Promise<void> {
    ...
    const index = this.virtualDb.findIndex(s => s.id === id);
    if (index !== -1) {
      this.virtualDb[index].isDeleted = false;
      this.virtualDb[index].deletedAt = undefined;
    }
  }

  async hardDelete(id: string): Promise<void> {
    await this.delay();
    this.virtualDb = this.virtualDb.filter(s => s.id !== id);
  }
}
