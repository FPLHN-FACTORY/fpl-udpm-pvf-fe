<template>
  <div class="space-y-4">
    <!-- View: Danh sách Sinh hoạt phí -->
    <template v-if="view === 'list'">
      <!-- Current Balance Section -->
      <div class="bg-white rounded-[6px] border border-[#E4E6E7] shadow-[0_2px_8px_rgba(0,0,0,0.08)] mb-4">
        <div class="pt-4 pb-2 px-5 flex items-center justify-between">
          <h2 class="font-bold text-base text-[#394552] m-0">Sinh hoạt phí</h2>
          <div class="flex gap-2">
            <a-button class="rounded-[6px] bg-[#EEEDF0] !text-[#384551] hover:!text-[#384551] border-none hover:bg-[#dcdbdc] transition-colors flex items-center justify-center">
              ← Quay Lại
            </a-button>
            <a-button
        ...
        <div class="px-5 pb-5 pt-2">
          <div class="flex items-center border border-[#E4E6E7] rounded-[6px] py-2.5 px-4 bg-white relative">
            <span class="text-[13px] font-bold text-[#394552]">Số dư hiện tại</span>
            <span class="text-[13px] text-[#394552] absolute left-1/2 -translate-x-1/2">10.000.000 VND</span>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-[6px] border border-[#E4E6E7] shadow-[0_2px_8px_rgba(0,0,0,0.08)]">
        <div class="py-4 px-5 border-b border-[#E4E6E7] flex justify-between items-center">
          <h2 class="font-bold text-base text-[#394552] m-0">Danh sách Yêu cầu rút sinh hoạt phí</h2>
        ...
        <div class="py-3 px-5 flex flex-wrap gap-3 items-center border-b border-[#E4E6E7]">
          ...
          <a-select
            ...
            class="w-48 rounded-md"
          >
            <a-select-option value="admin">Trần Minh Quân</a-select-option>
          </a-select>
          <a-select
    ...
    <!-- View: Chi tiết Yêu cầu rút sinh hoạt phí -->
    <template v-else>
      <div class="bg-white rounded-[6px] border border-[#E4E6E7] shadow-[0_2px_8px_rgba(0,0,0,0.08)] p-5 mb-4">
        <div class="flex items-center justify-between mb-5">
          <h2 class="font-bold text-base text-[#394552] m-0">Chi tiết Yêu cầu rút sinh hoạt phí</h2>
          <div class="flex gap-2 items-center">
            ...
            <tbody>
              ...
              >
                ...
                <td class="px-2 py-3 text-[13px] text-[#394552] align-middle">
                  <span v-if="row.key === 'status'" class="text-[#394552]">
                    {{ row.value }}
                  </span>
                  <template v-else>
                    {{ row.value || '—' }}
      ...
      <div class="bg-white rounded-[6px] border border-[#E4E6E7] shadow-[0_2px_8px_rgba(0,0,0,0.08)] p-5">
        <h3 class="font-bold text-base text-[#394552] mb-4">Lịch sử xử lý yêu cầu</h3>
        <div class="overflow-x-auto">
          <a-table 
            ...
            <template #bodyCell="{ column, record }">
              ...
              <template v-else-if="column.key === 'user'">
                <div class="flex items-center gap-3">
                  <img :src="record.userAvatar" class="w-8 h-8 rounded-full object-cover" alt="" />
                  <div>
                    <div class="font-medium text-[#394552]">{{ record.userName }}</div>
                    <div class="text-xs text-gray-500">{{ record.userEmail }}</div>
                  </div>
                </div>
              </template>
              <template v-else-if="column.key === 'actionType'">
                <span :class="record.objectType === 'Admin' ? 'text-gray-500' : (record.actionType === 'Tạo' ? 'text-[#FFAB00]' : record.actionType === 'Duyệt' ? 'text-[#FF3E1D]' : 'text-gray-500')">
                  {{ record.actionType }}
                </span>
              </template>
              <template v-else-if="column.key === 'objectType'">
                <span :class="[
                  ...
                  record.objectType === 'USER' ? 'bg-[#E8F9DF] text-[#71DD37]' : 'bg-[#E7E7FF] text-[#696CFF]'
                ]">
                  {{ record.objectType }}
                </span>
              </template>
              <template v-else-if="column.key === 'action'">
                <div class="flex gap-3 text-[#22303E]">
                  <button class="hover:text-blue-500 transition-colors" title="Xem chi tiết">
                    <EyeOutlined class="text-lg" />
                  </button>
                </div>
              </template>
...
import { getLivingFeeBalance, getLivingFeeRequests, getLivingFeeRequestDetail, getLivingFeeRequestHistory } from '../../../../services/home/api';
...
onMounted(async () => {
  ...
  try {
    ...
    const [fetchedBalance, fetchedRequests, fetchedDetail, fetchedHistory] = await Promise.all([
      getLivingFeeBalance(studentId),
      getLivingFeeRequests(studentId),
      getLivingFeeRequestDetail('HS001'),
      getLivingFeeRequestHistory('HS001')
    ]);
    balance.value = fetchedBalance;
    requests.value = fetchedRequests;
    detailData.value = fetchedDetail;
    historyRecords.value = fetchedHistory;
  } catch (error) {
    console.error('Lỗi khi lấy dữ liệu sinh hoạt phí:', error);
  } finally {
...
const detailRows = computed(() => [
  ...
  { key: 'student',      label: 'Học viên',                       value: detailData.value?.student },
  { key: 'contract',     label: 'Hợp đồng',                       value: detailData.value?.contract },
  { key: 'amount',       label: 'Số tiền đề nghị rút',            value: detailData.value?.amount },
  { key: 'purpose',      label: 'Mục đích/ghi chú rút',           value: detailData.value?.purpose },
  { key: 'file',         label: 'File minh chứng chi/rút (link/filename)', value: detailData.value?.file },
  { key: 'status',       label: 'Trạng thái xử lý đơn',           value: detailData.value?.status },
  { key: 'balanceBefore',label: 'Số dư trước khi rút',            value: detailData.value?.balanceBefore },
  { key: 'balanceAfter', label: 'Số dư sau khi duyệt',            value: detailData.value?.balanceAfter },
  { key: 'createdAt',    label: 'Ngày tạo yêu cầu',               value: detailData.value?.createdAt },
  { key: 'processedAt',  label: 'Ngày xử lý',                     value: detailData.value?.processedAt },
  { key: 'handler',      label: 'Người xử lý',                    value: detailData.value?.handler },
  { key: 'note',         label: 'Ghi chú nội bộ',                 value: detailData.value?.note },
]);

const historyColumns = [
  { title: '#', dataIndex: 'index', key: 'index', align: 'center', width: 80 },
  { title: 'THỜI GIAN', dataIndex: 'time', key: 'time' },
  { title: 'NGƯỜI DÙNG', key: 'user' },
  { title: 'HÀNH ĐỘNG', key: 'actionType' },
  { title: 'LOẠI ĐỐI TƯỢNG', key: 'objectType' },
