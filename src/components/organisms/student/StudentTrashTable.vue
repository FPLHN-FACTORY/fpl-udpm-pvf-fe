<template>
  <div class="bg-white rounded-md shadow-md">
    ...
    <div class="py-4 px-5 border-b border-[#E4E6E7] flex flex-nowrap gap-4 items-center justify-between overflow-x-auto">
      <h3 class="text-lg font-bold text-[#22303E] m-0 whitespace-nowrap">Danh sách Học viên đã xóa</h3>
      <div class="flex gap-2 flex-nowrap">
        <a-button type="default" @click="$emit('back')" style="background-color: #EBEDF0; border: none;" class="flex items-center gap-[2px] text-[13px] px-2 md:px-3 rounded-md !text-[#8592A3] hover:!text-[#8592A3] hover:bg-[#dcdbdc] transition-colors">
      ...
      <template #bodyCell="{ column, record }">
        ...
        <template v-else-if="column.key === 'action'">
          <div class="flex gap-3 text-[#22303E]">
            ...
            <button @click="$emit('restore', record)" class="hover:text-orange-500 transition-colors" title="Khôi phục">
              <UndoOutlined class="text-lg" />
            </button>
            <button @click="$emit('hardDelete', record)" class="hover:text-red-500 transition-colors" title="Xóa vĩnh viễn">
...
defineEmits(['back', 'restore', 'hardDelete']);
