...
const originalData = {
  ...
  type: '1',
  startDate: dayjs('2025-01-15'),
  endDate: dayjs('2025-01-25'),
  status: 'active',
  terms: [{ name: 'Điều khoản 1', desc: 'Nội dung chi tiết 1' }],
  allowances: [{ year: dayjs('2025', 'YYYY'), amount: '2000000' }],
  equipments: [{ type: '1', year: dayjs('2025', 'YYYY'), qty: '2' }]
}
...
const handleReset = () => {
  ...
  form.type = originalData.type as any
  form.startDate = originalData.startDate
  form.endDate = originalData.endDate
  form.status = originalData.status as any
  form.terms = JSON.parse(JSON.stringify(originalData.terms))
  
  // Note: parsing JSON removes dayjs objects, so need to map back
  form.allowances = originalData.allowances.map(a => ({ year: a.year, amount: a.amount }))
  form.equipments = originalData.equipments.map(e => ({ type: e.type, year: e.year, qty: e.qty }))
}
</script>
