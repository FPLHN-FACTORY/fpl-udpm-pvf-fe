<template>
  <div class="flex flex-col gap-6 p-6 min-h-screen bg-[#f5f5f9]">
    ...
    <div class="flex items-center gap-2 text-sm mb-2">
      ...
      <span class="text-gray-400">/</span>
      <span class="text-[#566a7f] font-medium">Thêm mới</span>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden max-w-5xl mx-auto w-full">
      ...
      <div class="flex justify-between items-center p-6 border-b border-gray-100">
        <h2 class="text-lg font-bold text-[#566a7f]">Thêm Môn học ngoại khoá</h2>
        <a-button 
      ...
      <div class="p-8 flex flex-col gap-6">
        <div>
          <a-input v-model:value="formState.code" placeholder="Mã môn" class="!h-10 !border-[#d9dee3] rounded-md" />
        </div>
        <div>
...
const handleSubmit = async () => {
  if (!formState.value.code || !formState.value.name || !formState.value.status) {
    message.error('Vui lòng điền các trường bắt buộc (Mã môn, Tên môn, Trạng thái)')
    return
  }

  loading.value = true
  try {
    await mhnkService.create(formState.value)
